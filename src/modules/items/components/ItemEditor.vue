...
const props = withDefaults(
  defineProps<{
    ...
    groups: Group[]
    saving?: boolean
    // Guest gating (spec 4.2): a guest may not edit others' items, nor assign to
    // anyone but themselves. Defaults keep owners/members unrestricted.
    canEdit?: boolean
    canAssignOthers?: boolean
    myMembershipId?: string | null
  }>(),
  { canEdit: true, canAssignOthers: true, myMembershipId: null },
)

// Read-only when the viewer may not edit this item; the form is then disabled
// and the actions collapse to a close button.
const editable = computed(() => props.canEdit)
// Who the assignee dropdown may offer: everyone, or only the viewer themselves.
...
<template>
  ...
  <div
    ...
    role="dialog"
    :aria-label="mode === 'create' ? 'Nieuw item' : editable ? 'Item bewerken' : 'Item bekijken'"
  >
    <div class="mx-auto mt-3 h-1 w-9 shrink-0 rounded-full bg-border" aria-hidden="true"></div>

    <fieldset class="contents" :disabled="!editable">
      <div class="flex-1 overflow-y-auto px-5 pb-4 pt-3">
        ...
        <label class="mt-4 flex flex-col gap-1.5">
          ...
          >
            ...
            <option v-for="m in assignableMembers" :key="m.membershipId" :value="m.membershipId">{{ m.name }}</option>
          </select>
        </label>
    ...
    <div class="flex flex-col gap-2 border-t border-border px-5 pb-7 pt-3">
      ...
      <template v-else>
        <p class="text-body2 text-muted">Alleen-lezen — je kunt dit item niet bewerken.</p>
        <button
          type="button"
          class="h-[52px] rounded-card bg-surface2 text-lg font-medium text-text"
          @click="emit('close')"
        >
          Sluiten
        </button>
